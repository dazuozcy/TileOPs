...
- **MultiHeadLatentAttentionDecodeWithKVCacheFwdOp** — DeepSeek MLA decode with KV cache `o = mla_decode(q, q_pe, k, k_pe)` (NPU kernel placeholder).
...
| `MLADecodeKernel` | `kernels/deepseek_mla_decode.py` | TileLang (NPU) | STUB (empty body) — compiles and runs; output values undefined until MLA decode forward body is implemented |
