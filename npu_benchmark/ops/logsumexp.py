...
from kernels.logsumexp import LogSumExpKernel
from perf.formulas import logsumexp_fwd_roofline
...
class LogSumExpFwdOp(Op):
    ...
    Conforms to ``torch.logsumexp``. The Op layer moves the reduction
    dimension to the last axis, reshapes the input to a 2-D ``(M, N)``
    tensor (where ``N`` is the reduction-dim size and ``M`` is the
    product of all other dims), and dispatches the ``LogSumExpKernel``.

    Input:  x [...]
    Output: output [...]  (reduced shape depends on ``keepdim``)

    Backed by the TileLang ``LogSumExpKernel`` (NPU — currently a
    placeholder stub; see ``kernels/logsumexp.py``).
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if not is_available():
            raise ValueError("LogSumExpFwdOp expects an accelerator device")
        if x.dtype not in self._SUPPORTED_DTYPES:
            names = ", ".join(str(dt) for dt in self._SUPPORTED_DTYPES)
            raise ValueError(
                f"LogSumExpFwdOp does not support dtype {x.dtype}. "
                f"Supported: [{names}]"
            )
        if x.ndim == 0:
    ...
    def eval_roofline(self) -> tuple[int, int]:
        return logsumexp_fwd_roofline(self)
